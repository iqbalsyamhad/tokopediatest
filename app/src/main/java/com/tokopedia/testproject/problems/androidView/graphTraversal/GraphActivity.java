...
public class GraphActivity extends AppCompatActivity {
    ...
    private int[] nodeColor = new int[7];
    ...
    private void setupAdapter(Graph graph) {
        ...
        adapter = new BaseGraphAdapter<ViewHolder>(this, R.layout.node, graph) {
            ...
            public void onBindViewHolder(ViewHolder viewHolder, Object data, int position) {
                ...
                viewHolder.cardView.setCardBackgroundColor(nodeColor[position]);
            }
        };
    ...
    private void traverseAndColorTheGraph(Graph graph, Node rootNode, int target) {
        String root = rootNode.getData().toString();
        for (int i = 0; i < graph.getNodes().size(); i++) {
            int nodestep = 0;
            String destination = graph.getNodes().get(i).getData().toString();
            while (!destination.equals(root)) {
                for (int j = 0; j < graph.getEdges().size(); j++) {
                    if (graph.getEdges().get(j).getDestination().getData().toString().equals(destination)) {
                        destination = graph.getEdges().get(j).getSource().getData().toString();
                        nodestep++;
                        break;
                    }
                }
            ...
            if (nodestep == 0) {
                nodeColor[i] = Color.parseColor("#6A0DAD");
            } else if (nodestep < target) {
                nodeColor[i] = Color.GREEN;
            } else if (nodestep == target) {
                nodeColor[i] = Color.parseColor("#FFA500");
            } else {
